<artifact identifier="document-submit-improved" type="application/vnd.ant.code" language="php" title="HR Service Form - Improved Version with All Features">
<?php
// /views/qr_form/document_submit.php - IMPROVED VERSION
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $employeeId = trim($_POST['employee_id'] ?? ''); // แก้ไขจุดที่ 2
...
<main class="max-w-7xl mx-auto px-4 py-6 md:py-8">
    <?php if ($message): ?>
        <div class="mb-6 p-4 rounded-lg animate-pulse <?php echo $messageType === 'success' ? 'bg-green-50 dark:bg-green-900/20 border-l-4 border-green-500' : 'bg-red-50 dark:bg-red-900/20 border-l-4 border-red-500'; ?>">
            <p class="<?php echo $messageType === 'success' ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400'; ?> font-semibold">
                <?php echo htmlspecialchars($message); ?>
            </p>
            <?php if ($messageType === 'success'): ?>
                <button onclick="resetForm()" class="mt-2 text-sm text-green-600 dark:text-green-400 hover:text-green-800 dark:hover:text-green-300 underline">
                    ส่งคำขอใหม่ →
                </button>
            <?php endif; ?>
        </div>
        ...
        <form method="POST" enctype="multipart/form-data" id="submissionForm" class="p-6 space-y-6">
            ...
            <div>
                ...
                <textarea id="notes" 
                          name="notes" 
                          rows="4"
                          class="w-full px-4 py-3 border-2 border-blue-200 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition dark:bg P-gray-700 dark:text-white"
